...
elseif($request_method == 'POST')
{
	...
	if($response_array['return_code']==1 && $response_array['return_message']=="New Record Inserted Successfully."){
			
		header("location:../index.php?page=mydeals");
	}	
	elseif($response_array['return_message']=="Already Accepted this deal!"){
			$prev="javascript:history.go(-1)";
			if(isset($_SERVER['HTTP_REFERER'])){$prev=$_SERVER['HTTP_REFERER'];}
			header("location:".$prev."&msg=".$response_array['return_message']);
	}
